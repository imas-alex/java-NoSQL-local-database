...
public class JNwCustomerEditor extends JFieldsEditor {

	private static final long serialVersionUID = 1L;
	boolean debug=false;
	public JNwCustomerEditor() {
		super();
	}
	@Override
	public String getLocator() {
		try{
			...
			locator.setProperty(BaseHandler.HANDLER_CLASS,JNwCustomerEditor.class.getName());
	...
	public String getCategoryIcon(Entigrator entigrator) {
		return ExtensionHandler.loadIcon(entigrator, NwCustomerHandler.EXTENSION_KEY, "nwCustomer.png");
	}
	...
	public void reindex(JMainConsole console, Entigrator entigrator, Sack entity) {
		 try{
			// System.out.println("JPhoneEditor:reindex:0:entity="+entity.getProperty("label"));
		    	String fhandler$=NwCustomerHandler.class.getName();
		    	if(entity.getElementItem("fhandler", fhandler$)!=null){
		    		entity.putElementItem("jfacet", new Core(JNwCustomerFacetAddItem.class.getName(),fhandler$,JNwCustomerFacetOpenItem.class.getName()));
					entity.putElementItem("fhandler", new Core(null,fhandler$,NwCustomerHandler.EXTENSION_KEY));
	...
	public String newEntity(JMainConsole console, String locator$) {
	    ...
	    editorLocator$=Locator.append(editorLocator$, JTextEditor.TEXT, "nwCustomer"+Identity.key().substring(0,4));
	    editorLocator$=Locator.append(editorLocator$,Locator.LOCATOR_TITLE,"nwCustomer entity");
	    JNwCustomerEditor fe=new JNwCustomerEditor();
	   ...
	   responseLocator.setProperty(BaseHandler.HANDLER_CLASS,JNwCustomerEditor.class.getName());
		...
		responseLocator.setProperty(Locator.LOCATOR_TITLE,"nwCustomer");
	...
	//	System.out.println("JAddressEditor:response:"+Locator.remove(locator$,Locator.LOCATOR_ICON ));
		try{
			...
			if(ACTION_NEW_ENTITY.equals(action$)){
				...
				newEntity.putElementItem("fhandler", new Core(null,NwCustomerHandler.class.getName(),NwCustomerHandler.EXTENSION_KEY));
				...
				newEntity.putAttribute(new Core (null,"icon","nwCustomer.png"));
				entigrator.replace(newEntity);
				entigrator.ent_assignProperty(newEntity, "fields", text$);
				entigrator.ent_assignProperty(newEntity, "nwCustomer", text$);
				String icons$=entihome$+"/"+Entigrator.ICONS;
				Support.addHandlerIcon(JNwCustomerEditor.class, "nwCustomer.png", icons$);
