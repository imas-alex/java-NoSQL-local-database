package gdt.jgui.entity.nwproduct;
/*
...
import gdt.data.entity.NwProductHandler;
...
public class JNwProductFacetAddItem extends JFacetAddItem{
	private static final long serialVersionUID = 1L;
	//public static final String EXTENSION_KEY="_v6z8CVgemqMI6Bledpc7F1j0pVY";
	private Logger LOGGER=Logger.getLogger(JNwProductFacetAddItem.class.getName());
    String entityLabel$;
    static boolean debug=false;
    public JNwProductFacetAddItem(){
		super();
	}
@Override
public String getLocator(){
	Properties locator=new Properties();
	locator.setProperty(Locator.LOCATOR_TITLE,"nwProduct");
	locator.setProperty(BaseHandler.HANDLER_CLASS,JNwProductFacetAddItem.class.getName());
	locator.setProperty(BaseHandler.HANDLER_SCOPE,JConsoleHandler.CONSOLE_SCOPE);
	locator.setProperty(BaseHandler.HANDLER_METHOD,METHOD_ADD_COMPONENT);
	locator.setProperty(BaseHandler.HANDLER_LOCATION,NwProductHandler.EXTENSION_KEY);
	locator.setProperty( JContext.CONTEXT_TYPE,"nwProduct add ");
	locator.setProperty(Locator.LOCATOR_TITLE,"nwproduct");
	locator.setProperty(JFacetOpenItem.FACET_HANDLER_CLASS,NwProductHandler.class.getName());
	if(entityKey$!=null)
	...
	locator.setProperty( Locator.LOCATOR_ICON_FILE, "nwProduct.png");
...
//	System.out.println("JFieldsFacetAddItem:response:locator:"+locator$);
	try{
		...
		if(JFacetAddItem.ADD_MODE_COMPONENT.equals(mode$)){
			...
			Sack component=entigrator.ent_new("nwProduct", componentLabel$);
			component.createElement("fhandler");
			component.putElementItem("fhandler", new Core(null,NwProductHandler.class.getName(),null));
			component.createElement("jfacet");
			component.putElementItem("jfacet", new Core(JNwProductFacetAddItem.class.getName(),NwProductHandler.class.getName(),JNwProductFacetOpenItem.class.getName()));
			component.createElement("field");
			component.putElementItem("field", new Core(null,"Name","Value"));
			entigrator.save(component);
			entigrator.saveHandlerIcon(JNwProductFacetAddItem.class, "nwProduct.png");
...
public void addFacet(JMainConsole console, String locator$) {
	...
	//   System.out.println("JFieldsFacetAddItem:addFacet:locator:"+locator$);
	   ...
	   entity.putElementItem("jfacet", new Core(JNwProductFacetAddItem.class.getName(),NwProductHandler.class.getName(),JNwProductFacetOpenItem.class.getName()));
	   entigrator.save(entity);
	   entity=entigrator.ent_assignProperty(entity, "fields", entity.getProperty("label"));
	   entity=entigrator.ent_assignProperty(entity, "nwProduct", entity.getProperty("label"));
	}catch(Exception e){
		  LOGGER.severe(e.toString());
	  }
}
...
public void addComponent(JMainConsole console, String locator$) {
	try{
		if(debug)
			System.out.println("JNwProductFacetAddItem:addComponent:locator="+locator$);
		Properties locator=Locator.toProperties(locator$);
	    ...
	    editorLocator$=Locator.append(editorLocator$,JTextEditor.TEXT_TITLE,"Add nwProduct component");
	    if(entihome$!=null)
	    ...
	    responseLocator$=Locator.append(responseLocator$, BaseHandler.HANDLER_LOCATION,NwProductHandler.EXTENSION_KEY);
...
public String  markAppliedUncheckable(JMainConsole console,String locator$) {
	try{
		...
		if(entity.getProperty("address")!=null){
			Core fhandler=entity.getElementItem("fhandler",NwProductHandler.class.getName());
			if(fhandler!=null){
					Core jfacet=entity.getElementItem("jfacet", NwProductHandler.class.getName());
					if(jfacet!=null){
						if( JNwProductFacetOpenItem.class.getName().equals(jfacet.value)
								&& JNwProductFacetAddItem.class.getName().equals(jfacet.type)){
							isApplied=true;	
						}
					}
			}
...
public String getIconResource() {
	return "nwProduct.png";
}
@Override
public String getFacetOpenClass() {
	return JNwProductFacetOpenItem.class.getName();
}
@Override
public String getFacetAddClass() {
	return JNwProductFacetAddItem.class.getName();
}
}
