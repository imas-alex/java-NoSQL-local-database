...
public class JNwProductEditor extends JFieldsEditor {

	private static final long serialVersionUID = 1L;
	
	boolean debug=false;
	public JNwProductEditor() {
		super();
	}
	@Override
	public String getLocator() {
		try{
			...
			locator.setProperty(BaseHandler.HANDLER_CLASS,JNwProductEditor.class.getName());
	...
	public String getCategoryIcon(Entigrator entigrator) {
		return ExtensionHandler.loadIcon(entigrator, NwProductHandler.EXTENSION_KEY, "nwProduct.png");
	...
	public void reindex(JMainConsole console, Entigrator entigrator, Sack entity) {
		 try{
			// System.out.println("JPhoneEditor:reindex:0:entity="+entity.getProperty("label"));
		    	String fhandler$=NwProductHandler.class.getName();
		    	if(entity.getElementItem("fhandler", fhandler$)!=null){
		    		entity.putElementItem("jfacet", new Core(JNwProductFacetAddItem.class.getName(),fhandler$,JNwProductFacetOpenItem.class.getName()));
					entity.putElementItem("fhandler", new Core(null,fhandler$,NwProductHandler.EXTENSION_KEY));
	...
	public String newEntity(JMainConsole console, String locator$) {
	    ...
	    editorLocator$=Locator.append(editorLocator$, JTextEditor.TEXT, "nwProduct"+Identity.key().substring(0,4));
	    editorLocator$=Locator.append(editorLocator$,Locator.LOCATOR_TITLE,"nwProduct entity");
	    JNwProductEditor fe=new JNwProductEditor();
	   ...
	   responseLocator.setProperty(BaseHandler.HANDLER_CLASS,JNwProductEditor.class.getName());
		...
		responseLocator.setProperty(Locator.LOCATOR_TITLE,"nwProduct");
	...
	//	System.out.println("JAddressEditor:response:"+Locator.remove(locator$,Locator.LOCATOR_ICON ));
		try{
			...
			if(ACTION_NEW_ENTITY.equals(action$)){
				...
				newEntity.putElementItem("fhandler", new Core(null,NwProductHandler.class.getName(),NwProductHandler.EXTENSION_KEY));
				...
				newEntity.putAttribute(new Core (null,"icon","nwProduct.png"));
				entigrator.replace(newEntity);
				entigrator.ent_assignProperty(newEntity, "fields", text$);
				entigrator.ent_assignProperty(newEntity, "nwProduct", text$);
				String icons$=entihome$+"/"+Entigrator.ICONS;
				Support.addHandlerIcon(JNwProductEditor.class, "nwProduct.png", icons$);
