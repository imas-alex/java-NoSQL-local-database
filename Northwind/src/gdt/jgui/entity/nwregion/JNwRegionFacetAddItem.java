package gdt.jgui.entity.nwregion;
/*
...
import gdt.data.entity.NwRegionHandler;
...
public class JNwRegionFacetAddItem extends JFacetAddItem{
	private static final long serialVersionUID = 1L;
	//public static final String EXTENSION_KEY="_v6z8CVgemqMI6Bledpc7F1j0pVY";
	private Logger LOGGER=Logger.getLogger(JNwRegionFacetAddItem.class.getName());
    String entityLabel$;
    static boolean debug=false;
    public JNwRegionFacetAddItem(){
		super();
	}
@Override
public String getLocator(){
	Properties locator=new Properties();
	locator.setProperty(Locator.LOCATOR_TITLE,"nwRegion");
	locator.setProperty(BaseHandler.HANDLER_CLASS,JNwRegionFacetAddItem.class.getName());
	locator.setProperty(BaseHandler.HANDLER_SCOPE,JConsoleHandler.CONSOLE_SCOPE);
	locator.setProperty(BaseHandler.HANDLER_METHOD,METHOD_ADD_COMPONENT);
	locator.setProperty(BaseHandler.HANDLER_LOCATION,NwRegionHandler.EXTENSION_KEY);
	locator.setProperty( JContext.CONTEXT_TYPE,"nwRegion add ");
	locator.setProperty(Locator.LOCATOR_TITLE,"nwRegion");
	locator.setProperty(JFacetOpenItem.FACET_HANDLER_CLASS,NwRegionHandler.class.getName());
	if(entityKey$!=null)
	...
	locator.setProperty( Locator.LOCATOR_ICON_FILE, "nwRegion.png");
...
//	System.out.println("JFieldsFacetAddItem:response:locator:"+locator$);
	try{
		...
		if(JFacetAddItem.ADD_MODE_COMPONENT.equals(mode$)){
			...
			Sack component=entigrator.ent_new("nwRegion", componentLabel$);
			component.createElement("fhandler");
			component.putElementItem("fhandler", new Core(null,NwRegionHandler.class.getName(),null));
			component.createElement("jfacet");
			component.putElementItem("jfacet", new Core(JNwRegionFacetAddItem.class.getName(),NwRegionHandler.class.getName(),JNwRegionFacetOpenItem.class.getName()));
			component.createElement("field");
			component.putElementItem("field", new Core(null,"Name","Value"));
			entigrator.save(component);
			entigrator.saveHandlerIcon(JNwRegionFacetAddItem.class, "nwRegion.png");
...
public void addFacet(JMainConsole console, String locator$) {
	...
	//   System.out.println("JFieldsFacetAddItem:addFacet:locator:"+locator$);
	   ...
	   entity.putElementItem("jfacet", new Core(JNwRegionFacetAddItem.class.getName(),NwRegionHandler.class.getName(),JNwRegionFacetOpenItem.class.getName()));
	   entigrator.save(entity);
	   entity=entigrator.ent_assignProperty(entity, "fields", entity.getProperty("label"));
	   entity=entigrator.ent_assignProperty(entity, "nwRegion", entity.getProperty("label"));
	}catch(Exception e){
		  LOGGER.severe(e.toString());
	  }
}
...
public void addComponent(JMainConsole console, String locator$) {
	try{
		if(debug)
			System.out.println("JNwRegionFacetAddItem:addComponent:locator="+locator$);
		Properties locator=Locator.toProperties(locator$);
	    ...
	    editorLocator$=Locator.append(editorLocator$, JTextEditor.TEXT, label$+".region."+Identity.key().substring(0,4));
	    editorLocator$=Locator.append(editorLocator$,Locator.LOCATOR_TITLE,"Component label");
	    editorLocator$=Locator.append(editorLocator$,JTextEditor.TEXT_TITLE,"Add nwRegion component");
	    if(entihome$!=null)
	    ...
	    responseLocator$=Locator.append(responseLocator$, BaseHandler.HANDLER_LOCATION,NwRegionHandler.EXTENSION_KEY);
...
public String  markAppliedUncheckable(JMainConsole console,String locator$) {
	try{
		...
		if(entity.getProperty("nwRegion")!=null){
			Core fhandler=entity.getElementItem("fhandler",NwRegionHandler.class.getName());
			if(fhandler!=null){
					Core jfacet=entity.getElementItem("jfacet", NwRegionHandler.class.getName());
					if(jfacet!=null){
						if( JNwRegionFacetOpenItem.class.getName().equals(jfacet.value)
								&& JNwRegionFacetAddItem.class.getName().equals(jfacet.type)){
							isApplied=true;	
						}
					}
			}
...
public String getFacetOpenClass() {
	return JNwRegionFacetOpenItem.class.getName();
}
@Override
public String getFacetAddClass() {
	return JNwRegionFacetAddItem.class.getName();
}
}
