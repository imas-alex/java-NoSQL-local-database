package gdt.jgui.entity.nworderdetail;
/*
...
import gdt.data.entity.NwOrderDetailHandler;
...
public class JNwOrderDetailFacetAddItem extends JFacetAddItem{
	private static final long serialVersionUID = 1L;
	private Logger LOGGER=Logger.getLogger(JNwOrderDetailFacetAddItem.class.getName());
    String entityLabel$;
    static boolean debug=false;
    public JNwOrderDetailFacetAddItem(){
		super();
	}
@Override
public String getLocator(){
	...
	locator.setProperty(BaseHandler.HANDLER_CLASS,JNwOrderDetailFacetAddItem.class.getName());
	locator.setProperty(BaseHandler.HANDLER_SCOPE,JConsoleHandler.CONSOLE_SCOPE);
	locator.setProperty(BaseHandler.HANDLER_METHOD,METHOD_ADD_COMPONENT);
	locator.setProperty(BaseHandler.HANDLER_LOCATION,NwOrderDetailHandler.EXTENSION_KEY);
	locator.setProperty( JContext.CONTEXT_TYPE,"nwOrderDetail add ");
	locator.setProperty(Locator.LOCATOR_TITLE,"nwOrderDetail");
	locator.setProperty(JFacetOpenItem.FACET_HANDLER_CLASS,NwOrderDetailHandler.class.getName());
	if(entityKey$!=null)
	...
	locator.setProperty( Locator.LOCATOR_ICON_FILE, "nwOrderDetail.png");
...
//	System.out.println("JFieldsFacetAddItem:response:locator:"+locator$);
	try{
		...
		if(JFacetAddItem.ADD_MODE_COMPONENT.equals(mode$)){
			...
			Sack component=entigrator.ent_new("nwOrderDetail", componentLabel$);
			component.createElement("fhandler");
			component.putElementItem("fhandler", new Core(null,NwOrderDetailHandler.class.getName(),null));
			component.createElement("jfacet");
			component.putElementItem("jfacet", new Core(JNwOrderDetailFacetAddItem.class.getName(),NwOrderDetailHandler.class.getName(),JNwOrderDetailFacetOpenItem.class.getName()));
			component.createElement("field");
			component.putElementItem("field", new Core(null,"Name","Value"));
			entigrator.save(component);
			entigrator.saveHandlerIcon(JNwOrderDetailFacetAddItem.class, "nwOrderDetail.png");
...
public void addFacet(JMainConsole console, String locator$) {
	...
	//   System.out.println("JFieldsFacetAddItem:addFacet:locator:"+locator$);
	   ...
	   entity.putElementItem("jfacet", new Core(JNwOrderDetailFacetAddItem.class.getName(),NwOrderDetailHandler.class.getName(),JNwOrderDetailFacetOpenItem.class.getName()));
	   entigrator.save(entity);
	   entity=entigrator.ent_assignProperty(entity, "fields", entity.getProperty("label"));
	   entity=entigrator.ent_assignProperty(entity, "nwOrderDetail", entity.getProperty("label"));
	}catch(Exception e){
		  LOGGER.severe(e.toString());
	  }
}
...
public void addComponent(JMainConsole console, String locator$) {
	try{
		if(debug)
			System.out.println("JNwOrderDetailFacetAddItem:addComponent:locator="+locator$);
		Properties locator=Locator.toProperties(locator$);
	    ...
	    responseLocator$=Locator.append(responseLocator$, BaseHandler.HANDLER_LOCATION,NwOrderDetailHandler.EXTENSION_KEY);
...
public FacetHandler getFacetHandler() {
	return new NwOrderDetailHandler();
}
@Override
public String  markAppliedUncheckable(JMainConsole console,String locator$) {
	try{
		...
		if(entity.getProperty("nwOrderDetail")!=null){
			Core fhandler=entity.getElementItem("fhandler",NwOrderDetailHandler.class.getName());
			if(fhandler!=null){
					Core jfacet=entity.getElementItem("jfacet", NwOrderDetailHandler.class.getName());
					if(jfacet!=null){
						if( JNwOrderDetailFacetOpenItem.class.getName().equals(jfacet.value)
								&& JNwOrderDetailFacetAddItem.class.getName().equals(jfacet.type)){
							isApplied=true;	
						}
					}
			}
...
public String getIconResource() {
	return "nwOrderDetail.png";
}
@Override
public String getFacetOpenClass() {
	return JNwOrderDetailFacetOpenItem.class.getName();
}
@Override
public String getFacetAddClass() {
	return JNwOrderDetailFacetAddItem.class.getName();
}
}
