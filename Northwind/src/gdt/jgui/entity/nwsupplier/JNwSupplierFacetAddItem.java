...
public class JNwSupplierFacetAddItem extends JFacetAddItem{
	private static final long serialVersionUID = 1L;
	//public static final String EXTENSION_KEY="_v6z8CVgemqMI6Bledpc7F1j0pVY";
	private Logger LOGGER=Logger.getLogger(JNwSupplierFacetAddItem.class.getName());
    String entityLabel$;
    static boolean debug=false;
    public JNwSupplierFacetAddItem(){
		super();
	}
@Override
public String getLocator(){
	Properties locator=new Properties();
	locator.setProperty(Locator.LOCATOR_TITLE,"nwSupplier");
	locator.setProperty(BaseHandler.HANDLER_CLASS,JNwSupplierFacetAddItem.class.getName());
	...
	locator.setProperty( JContext.CONTEXT_TYPE,"nwSupplier add ");
	locator.setProperty(Locator.LOCATOR_TITLE,"nwSupplier");
	locator.setProperty(JFacetOpenItem.FACET_HANDLER_CLASS,NwSupplierHandler.class.getName());
	if(entityKey$!=null)
...
//	System.out.println("JFieldsFacetAddItem:response:locator:"+locator$);
	try{
		...
		if(JFacetAddItem.ADD_MODE_COMPONENT.equals(mode$)){
			...
			Sack component=entigrator.ent_new("nwSupplier", componentLabel$);
			component.createElement("fhandler");
			component.putElementItem("fhandler", new Core(null,NwSupplierHandler.class.getName(),null));
			component.createElement("jfacet");
			component.putElementItem("jfacet", new Core(JNwSupplierFacetAddItem.class.getName(),NwSupplierHandler.class.getName(),JNwSupplierFacetOpenItem.class.getName()));
			component.createElement("field");
			component.putElementItem("field", new Core(null,"Name","Value"));
			entigrator.ent_alter(component);
			entigrator.saveHandlerIcon(JNwSupplierFacetAddItem.class, "nwSupplier.png");
...
public void addFacet(JMainConsole console, String locator$) {
	...
	//   System.out.println("JFieldsFacetAddItem:addFacet:locator:"+locator$);
	   ...
	   entity.putElementItem("jfacet", new Core(JNwSupplierFacetAddItem.class.getName(),NwSupplierHandler.class.getName(),JNwSupplierFacetOpenItem.class.getName()));
	   entigrator.ent_alter(entity);
	   entity=entigrator.ent_assignProperty(entity, "fields", entity.getProperty("label"));
	   entity=entigrator.ent_assignProperty(entity, "nwSupplier", entity.getProperty("label"));
	}catch(Exception e){
		  LOGGER.severe(e.toString());
	  }
}
...
public void addComponent(JMainConsole console, String locator$) {
	try{
		if(debug)
			System.out.println("JNwSupplierFacetAddItem:addComponent:locator="+locator$);
		Properties locator=Locator.toProperties(locator$);
	    ...
	    editorLocator$=Locator.append(editorLocator$, JTextEditor.TEXT, label$+".supplier."+Identity.key().substring(0,4));
	    editorLocator$=Locator.append(editorLocator$,Locator.LOCATOR_TITLE,"Component label");
	    editorLocator$=Locator.append(editorLocator$,JTextEditor.TEXT_TITLE,"Add nwSupplier component");
	    if(entihome$!=null)
	    ...
	    responseLocator$=Locator.append(responseLocator$, BaseHandler.HANDLER_LOCATION,NwSupplierHandler.EXTENSION_KEY);
...
public String  markAppliedUncheckable(JMainConsole console,String locator$) {
	try{
		...
		if(entity.getProperty("nwSupplier")!=null){
			Core fhandler=entity.getElementItem("fhandler",NwSupplierHandler.class.getName());
			if(fhandler!=null){
					Core jfacet=entity.getElementItem("jfacet", NwSupplierHandler.class.getName());
					if(jfacet!=null){
						if( JNwSupplierFacetOpenItem.class.getName().equals(jfacet.value)
								&& JNwSupplierFacetAddItem.class.getName().equals(jfacet.type)){
							isApplied=true;	
						}
					}
			}
...
public String getFacetOpenClass() {
	return JNwSupplierFacetOpenItem.class.getName();
}
@Override
public String getFacetAddClass() {
	return JNwSupplierFacetAddItem.class.getName();
}
}
