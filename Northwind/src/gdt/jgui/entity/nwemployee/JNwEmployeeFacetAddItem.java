...
public class JNwEmployeeFacetAddItem extends JFacetAddItem{
	private static final long serialVersionUID = 1L;
	private Logger LOGGER=Logger.getLogger(JNwEmployeeFacetAddItem.class.getName());
    String entityLabel$;
    static boolean debug=false;
    public JNwEmployeeFacetAddItem(){
		super();
	}
@Override
public String getLocator(){
	Properties locator=new Properties();
	locator.setProperty(Locator.LOCATOR_TITLE,"nwEmployee");
	locator.setProperty(BaseHandler.HANDLER_CLASS,JNwEmployeeFacetAddItem.class.getName());
	...
	locator.setProperty( JContext.CONTEXT_TYPE,"nwEmployee add ");
	locator.setProperty(Locator.LOCATOR_TITLE,"nwEmployee");
	locator.setProperty(JFacetOpenItem.FACET_HANDLER_CLASS,NwEmployeeHandler.class.getName());
	if(entityKey$!=null)
...
//	System.out.println("JFieldsFacetAddItem:response:locator:"+locator$);
	try{
		...
		if(JFacetAddItem.ADD_MODE_COMPONENT.equals(mode$)){
			...
			Sack component=entigrator.ent_new("nwEmployee", componentLabel$);
			component.createElement("fhandler");
			component.putElementItem("fhandler", new Core(null,NwEmployeeHandler.class.getName(),null));
			component.createElement("jfacet");
			component.putElementItem("jfacet", new Core(JNwEmployeeFacetAddItem.class.getName(),NwEmployeeHandler.class.getName(),JNwEmployeeFacetOpenItem.class.getName()));
			component.createElement("field");
			component.putElementItem("field", new Core(null,"Name","Value"));
			entigrator.save(component);
			entigrator.saveHandlerIcon(JNwEmployeeFacetAddItem.class, "nwEmployee.png");
...
public void addFacet(JMainConsole console, String locator$) {
	...
	//   System.out.println("JFieldsFacetAddItem:addFacet:locator:"+locator$);
	   ...
	   entity.putElementItem("jfacet", new Core(JNwEmployeeFacetAddItem.class.getName(),NwEmployeeHandler.class.getName(),JNwEmployeeFacetOpenItem.class.getName()));
	   entigrator.save(entity);
	   entity=entigrator.ent_assignProperty(entity, "fields", entity.getProperty("label"));
	   entity=entigrator.ent_assignProperty(entity, "nwEmployee", entity.getProperty("label"));
	}catch(Exception e){
		  LOGGER.severe(e.toString());
	  }
}
...
public void addComponent(JMainConsole console, String locator$) {
	try{
		if(debug)
			System.out.println("JNwEmployeeFacetAddItem:addComponent:locator="+locator$);
		Properties locator=Locator.toProperties(locator$);
	    ...
	    editorLocator$=Locator.append(editorLocator$,JTextEditor.TEXT_TITLE,"Add nwEmployee component");
	    if(entihome$!=null)
	    ...
	    responseLocator$=Locator.append(responseLocator$, BaseHandler.HANDLER_LOCATION,NwEmployeeHandler.EXTENSION_KEY);
...
public String  markAppliedUncheckable(JMainConsole console,String locator$) {
	try{
		...
		if(entity.getProperty("nwEmployee")!=null){
			Core fhandler=entity.getElementItem("fhandler",NwEmployeeHandler.class.getName());
			if(fhandler!=null){
					Core jfacet=entity.getElementItem("jfacet", NwEmployeeHandler.class.getName());
					if(jfacet!=null){
						if( JNwEmployeeFacetOpenItem.class.getName().equals(jfacet.value)
								&& JNwEmployeeFacetAddItem.class.getName().equals(jfacet.type)){
							isApplied=true;	
						}
					}
			}
...
public String getFacetAddClass() {
	return JNwEmployeeFacetAddItem.class.getName();
}
}
