...
/**
 * This class represents the fields facet in the list
...
public String getLocator(){
	...
	locator.setProperty( JContext.CONTEXT_TYPE,"Fields add ");
	locator.setProperty(Locator.LOCATOR_TITLE,"Fields");
	locator.setProperty(JFacetOpenItem.FACET_HANDLER_CLASS,FieldsHandler.class.getName());
	if(entityKey$!=null)
...
//	System.out.println("JFieldsFacetAddItem:response:locator:"+locator$);
	try{
		...
		if(JFacetAddItem.ADD_MODE_COMPONENT.equals(mode$)){
			...
			Sack component=entigrator.ent_new("fields", componentLabel$);
			component.createElement("fhandler");
			component.putElementItem("fhandler", new Core(null,FieldsHandler.class.getName(),null));
			component.createElement("jfacet");
			component.putElementItem("jfacet", new Core(getClass().getName(),FieldsHandler.class.getName(),JFieldsFacetOpenItem.class.getName()));
...
public String  markAppliedUncheckable(JMainConsole console,String locator$) {
	try{
		...
		if(entity.getProperty("fields")!=null){
			Core fhandler=entity.getElementItem("fhandler",FieldsHandler.class.getName());
			if(fhandler!=null){
					Core jfacet=entity.getElementItem("jfacet", FieldsHandler.class.getName());
					if(jfacet!=null){
						if( JFieldsFacetOpenItem.class.getName().equals(jfacet.value)
								&& JFieldsFacetAddItem.class.getName().equals(jfacet.type)){
							isApplied=true;	
						}
					}
			}
