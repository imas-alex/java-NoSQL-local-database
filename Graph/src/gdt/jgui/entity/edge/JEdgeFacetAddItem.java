...
public class JEdgeFacetAddItem extends JFacetAddItem{
	private static final long serialVersionUID = 1L;
	private Logger LOGGER=Logger.getLogger(JEdgeFacetAddItem.class.getName());
    String entityLabel$;
    public JEdgeFacetAddItem(){
		super();
	}
@Override
public String getLocator(){
	...
	locator.setProperty(JFacetOpenItem.FACET_HANDLER_CLASS,JEdgeFacetAddItem.class.getName());
	if(entityKey$!=null)
		locator.setProperty(EntityHandler.ENTITY_KEY,entityKey$);
	if(entihome$!=null){
		locator.setProperty(Entigrator.ENTIHOME,entihome$);
		Entigrator entigrator=console.getEntigrator(entihome$);
	// icon$=Support.readHandlerIcon(EdgeHandler.class, "edge.png");
	 icon$=ExtensionHandler.loadIcon(entigrator, EdgeHandler.EXTENSION_KEY, "edge.png");
	if(icon$!=null)
...
//	System.out.println("JFieldsFacetAddItem:response:locator:"+locator$);
	try{
		...
		if(JFacetAddItem.ADD_MODE_COMPONENT.equals(mode$)){
			...
			Entigrator entigrator=console.getEntigrator(entihome$);
			Sack component=entigrator.ent_new("edge", componentLabel$);
			component.createElement("fhandler");
			component.putElementItem("fhandler", new Core(null,EdgeHandler.class.getName(),null));
			component.createElement("jfacet");
			component.putElementItem("jfacet", new Core(JEdgeFacetAddItem.class.getName(),EdgeHandler.class.getName(),JEdgeFacetOpenItem.class.getName()));
			component.createElement("field");
			component.putElementItem("field", new Core(null,"Name","Value"));
			entigrator.save(component);
			entigrator.saveHandlerIcon(JEdgeFacetAddItem.class, "edge.png");
...
public void addFacet(JMainConsole console, String locator$) {
	...
	//   System.out.println("JFieldsFacetAddItem:addFacet:locator:"+locator$);
	   ...
	   entity.putElementItem("fhandler", new Core(null,EdgeHandler.class.getName(),EdgeHandler.EXTENSION_KEY)); 
	   if(!entity.existsElement("jfacet"))
		   entity.createElement("jfacet");
	   entity.putElementItem("jfacet", new Core(JEdgeFacetAddItem.class.getName(),EdgeHandler.class.getName(),JEdgeFacetOpenItem.class.getName()));
	   entigrator.save(entity);
	   entity=entigrator.ent_assignProperty(entity, "fields", entity.getProperty("label"));
	   entity=entigrator.ent_assignProperty(entity, "edge", entity.getProperty("label"));
	}catch(Exception e){
		  LOGGER.severe(e.toString());
	  }
}
...
public void addComponent(JMainConsole console, String locator$) {
	...
	try{
	    ...
	    editorLocator$=Locator.append(editorLocator$, JTextEditor.TEXT, label$+".edge."+Identity.key().substring(0,4));
	    editorLocator$=Locator.append(editorLocator$,Locator.LOCATOR_TITLE,"Component label");
	    editorLocator$=Locator.append(editorLocator$,JTextEditor.TEXT_TITLE,"Add edge component");
...
public String  markAppliedUncheckable(JMainConsole console,String locator$) {
	try{
		...
		if(entity.getProperty("edge")!=null){
			Core fhandler=entity.getElementItem("fhandler",EdgeHandler.class.getName());
			if(fhandler!=null){
					Core jfacet=entity.getElementItem("jfacet", EdgeHandler.class.getName());
					if(jfacet!=null){
						if( JEdgeFacetOpenItem.class.getName().equals(jfacet.value)
								&& JEdgeFacetAddItem.class.getName().equals(jfacet.type)){
							isApplied=true;	
						}
					}
			}
...
public String getFacetAddClass() {
	return JEdgeFacetAddItem.class.getName();
}
}
