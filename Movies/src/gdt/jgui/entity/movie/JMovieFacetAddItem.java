...
public class JMovieFacetAddItem extends JFacetAddItem{
	private static final long serialVersionUID = 1L;
	public static final String EXTENSION_KEY="_35a4Gr4U9MGmswmMRFtgK2erNo8";
	private Logger LOGGER=Logger.getLogger(JMovieFacetAddItem.class.getName());
    String entityLabel$;
    public JMovieFacetAddItem(){
		super();
	}
@Override
public String getLocator(){
	...
	locator.setProperty(BaseHandler.HANDLER_CLASS,JMovieFacetAddItem.class.getName());
	...
	locator.setProperty( JContext.CONTEXT_TYPE,"Movie add ");
	locator.setProperty(Locator.LOCATOR_TITLE,"Movie");
	locator.setProperty(JFacetOpenItem.FACET_HANDLER_CLASS,MovieHandler.class.getName());
	if(entityKey$!=null)
...
//	System.out.println("JMovieFacetAddItem:response:locator:"+locator$);
	try{
		...
		if(JFacetAddItem.ADD_MODE_COMPONENT.equals(mode$)){
			...
			Sack component=entigrator.ent_new("movie", componentLabel$);
			component.createElement("fhandler");
			component.putElementItem("fhandler", new Core(null,MovieHandler.class.getName(),null));
...
public String  markAppliedUncheckable(JMainConsole console,String locator$) {
	try{
		...
		if(entity.getProperty("movie")!=null){
			Core fhandler=entity.getElementItem("fhandler",MovieHandler.class.getName());
			if(fhandler!=null){
					Core jfacet=entity.getElementItem("jfacet", MovieHandler.class.getName());
					if(jfacet!=null){
						if( JMovieFacetOpenItem.class.getName().equals(jfacet.value)
								&& JMovieFacetAddItem.class.getName().equals(jfacet.type)){
							isApplied=true;	
						}
					}
			}
...
public String getFacetOpenClass() {
	return JMovieFacetOpenItem.class.getName();
}
@Override
public String getFacetAddClass() {
	return JMovieFacetAddItem.class.getName();
}
}
